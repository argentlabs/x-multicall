...
import { Methods as RPCMethods } from "@starknet-io/types-js"
...
type RpcRequest<T extends keyof RPCMethods = keyof RPCMethods> = {
  method: T
...
export class RpcChannelBatch extends RpcChannel {
  ...
  private loader:
    | DataLoader<RpcRequest, any /* RPC.ResponseBody[] */>
    | undefined
  ...
  private async batchRequests(
    ...
    canonicalRequests.sort(
      (a, b) => a.method.localeCompare(b.method) || a.id - b.id,
    )
    ...
    const extraHeaders = {
      ...(process.env.IS_PLAYWRIGHT === "true" && {
        "X-Playback-Key": playbackKey(bodyCanonical),
      }),
    ...
    // TODO: use correct type when exported from starknetjs
    const data: any /* RPC.Response[] */ | unknown = await response
      .json()
      .catch(async () => {
  ...
  protected errorHandler(
    ...
    if (rpcError) {
      const { code, message, baseError } = rpcError
      throw new LibraryError(
        `RPC: ${method} with params ${JSON.stringify(
          params,
        )}\n ${code}: ${message}: ${JSON.stringify(baseError.message)}`,
      )
    }
  ...
  protected async fetchEndpoint<T extends keyof RPCMethods>(
    method: T,
    params: RPCMethods[T]["params"],
